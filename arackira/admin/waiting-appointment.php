...
if (!empty($_GET["id"]) && !empty($_GET["status"])) {
    if ($_GET["status"] == 1) {
        ...
        header('Location: waiting-appointment.php?success=Successfully Deleted');
        exit;
    } else if ($_GET["status"] == 3) {

        $success_query = $db->prepare("Update appointment SET payment_status = 1 WHERE id = :id");
        $success_query->bindParam(':id', $_GET["id"]);
        $success_query->execute();
        header('Location: waiting-appointment.php?success=Payment Received Successfully');
        exit;
    }
}

$appointments = $db->prepare("SELECT * FROM appointment INNER JOIN  cars ON appointment.carID = cars.car_id WHERE appointment.status = 0");
                    ...
                    <tbody>
                        <?php foreach ($appointments as $appointment) : ?>
                            <tr>
                                ...
                                <td><?php echo $appointment["payment_type"] == 0 ? "Credit Card" : "Cash" ?> (<?php echo $appointment["payment_status"] == 1 ? "Completed" : "Pending" ?>)</td>
                                <td>
                                    <a href="comp/appointment.php?id=<?php echo $appointment['id'] ?>&status=1" class="btn btn-success">Confirm</a>
                                    <?php
                                    if ($appointment["payment_status"] == 0) {
                                        echo "<a href='comp/payment.php?id=" . $appointment['id'] . "&status=1' class='btn btn-warning'>Get Paid</a>";
                                    }
                                    ?>
                                    <a href="comp/appointment.php?id=<?php echo $appointment['id'] ?>&status=2" class="btn btn-danger">Reject</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
